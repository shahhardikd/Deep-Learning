...
def initialize_parameters(n_x, n_h, n_y):
    ...
    W1 = np.multiply(np.random.randn(n_h,n_x),0.01)
    b1 = np.zeros((n_h,1))
    W2 = np.multiply(np.random.randn(n_y,n_h),0.01)
    b2 = np.zeros((n_y,1))
    ### END CODE HERE ###
...
def compute_cost(A2, Y, parameters):
    ...
    cost = -np.multiply(np.divide(1,m),np.sum(logprobs))
    ### END CODE HERE ###
...
def backward_propagation(parameters, cache, X, Y):
    ...
    dZ2 = A2 - Y
    dW2 = np.multiply(np.divide(1,m),np.dot(dZ2,A1.T))
    db2 = (1/m)*np.sum(dZ2, keepdims=True,axis=1)
    dZ1 = np.multiply(np.dot(W2.T,dZ2),(1-np.power(A1,2)))
    dW1 = np.multiply(np.divide(1,m),np.dot(dZ1,X.T))
